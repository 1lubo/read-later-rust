...
async fn list_is_newest_first_and_filters_compose() {
    ...
    let read_only = store.list(&ListQuery { read: Some(true), ..Default::default() }).await.unwrap();
...
async fn mark_ready_and_failed_transition_status() {
    let store = InMemoryBookmarkStore::new();
    store.insert(bm("a", "https://a.test", &[], 1)).await.unwrap();
    store.insert(bm("b", "https://b.test", &[], 2)).await.unwrap();

    store.mark_ready("a", Some("Title".into()), Some("Excerpt".into())).await.unwrap();
